...
private:
    ...
    string generarCodigoReserva() {
        ...
        int numero = 10000 + (rand() % 90000);
        return "RES" + to_string(numero);
    }
...
public:
    ...
    static int compararFechas(const string& f1, const string& f2) {
        ...
        if (f1.length() < 10 || f2.length() < 10) {
            // Si alguna fecha es inválida, considerar la más corta como menor
            if (f1.length() != f2.length()) {
                return (f1.length() < f2.length()) ? -1 : 1;
            }
            return 0; // Ambas inválidas, considerarlas iguales
        }
